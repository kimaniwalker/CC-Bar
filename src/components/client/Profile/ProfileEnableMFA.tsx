...
export const ProfileEnableMFA = async ({ action_key, user_id }: { action_key: RewardActionKey, user_id: string | null }) => {
    ...
    const rewardsAction = data.find((item: UserRewardsActionsAndAccount) => item.reward_actions.key === action_key)

    const hasBeenCompleted = rewardsAction?.reward_actions.key === action_key


    return (<div className="mt-12 rounded-3xl border border-neutral-200 bg-[#F8F5F1] p-5 relative">

        {!hasBeenCompleted && (<BadgeCoin worth={REWARD_ACTIONS[RewardActionKey.ADD_PHONE].reward} className="absolute -top-4 -right-4" action_key={RewardActionKey.ADD_PHONE} user_id={user_id} />)}
