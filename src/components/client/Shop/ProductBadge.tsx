...
export const ProductBadge = ({ stock }: { stock: number }) => {
  if (getProductStockStatus(stock) === ProductAvailabilityStatus.OUT_OF_STOCK) {
    return (
      <span
        className={`absolute bottom-2 left-2 bg-red-600 text-white text-xs px-2 py-1 rounded ${montserrat.className}`}
      >
  ...
  if (getProductStockStatus(stock) === ProductAvailabilityStatus.LOW_STOCK) {
    return (
      <span
        className={`absolute bottom-2 left-2 bg-yellow-500 text-white text-xs px-2 py-1 rounded ${montserrat.className}`}
      >
  ...
  if (getProductStockStatus(stock) === ProductAvailabilityStatus.SELLING_FAST) {
    return (
      <span
        className={`absolute bottom-2 left-2 bg-orange-500 text-white text-xs px-2 py-1 rounded ${montserrat.className}`}
      >
