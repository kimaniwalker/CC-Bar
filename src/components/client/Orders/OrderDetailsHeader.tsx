  ...
  return (
    <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between rounded-3xl bg-white p-6 shadow-sm my-4">
      <div>
        ...
        <Text
          size="sm"
          className={`mt-2 text-sm font-semibold text-neutral-500`}
        >
          Order: {id.slice(0, 8)}
        </Text>
        ...
        <div>
          <Text
            as="span"
            size="sm"
            className={`mt-3 inline-flex items-center rounded-full px-3 py-1 text-sm font-medium uppercase ${statusStyles[status] || "bg-gray-100 text-gray-800 uppercase"}`}
          >
            {status}
          </Text>
        </div>
